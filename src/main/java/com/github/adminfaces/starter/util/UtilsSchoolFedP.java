...
import com.github.adminfaces.starter.bean.SchoolsFedPListMB;
...
public class UtilsSchoolFedP extends Db implements Serializable {

	/**
	 *
	 */
	private static Logger logger = Logger.getLogger(UtilsSchoolFedP.class);
	private static final long serialVersionUID = 1L;
	public static List<SchoolFedP> patients;
	...
	public void init() {
		...
		try {
			// To display date on menu selection
			String parsed_selected_date = SchoolsFedPListMB.dynamicDateFormater(originalFormat, targetFormat,
					items_date_toView);
		...
		for (int x = 0; x < finalArraylist.size(); x++) {
			for (int y = 0; y < innerArraylist.size(); y++) {
				try {
					SQL = "SELECT * FROM tbl_temis_items_updated_ecde where items_date = '" + currentDateYYYYMMDD()
	...
	@Produces
	public List<SchoolFedP> getPatients() {
		return patients;
	}

	public void setPatients(List<SchoolFedP> patients) {
		UtilsSchoolFedP.patients = patients;
	}
	...
	public void setItems_date_toView(String items_date_toView) {
		...
		UtilsSchoolFedP.items_date_toView = items_date_toView;
	}
